...
<template>
  ...
  <script>
  import xzqCode from "@/utils/xzqCode";
  import { currentGET } from "@/utils/model/api";
  import * as echarts from "echarts";
  import { GETNOBASE } from "@/utils/api";
  export default {
    name:'Map',
    ...
    methods: {
      getData(code) {
        currentGET("map", { regionCode: code }).then((res) => {
        if (res.code==200)
        {
            this.mydata=res.data
                // console.log('map',this.data)
             this.getGeojson(code, this.mydata);
            this.mapclick();
        }
        });
